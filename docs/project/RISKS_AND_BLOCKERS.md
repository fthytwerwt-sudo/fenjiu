...
| R-11 | **BLOCKED / real acquisition governance（真实获客治理）** | 已有客户定位规范，但仍缺重新启用范围的用户决定、允许来源及条款、联系人处理依据、DNC/删除/保留、私有存储、真实身份/RBAC、供应链批准事实和外发停止线 | 不得把候选客户类型、关键词或评分当作采集真实名单、处理联系人、CRM 写入或 Gmail 外发授权 | 先按 `NEPAL_CUSTOMER_TARGETING_SPECIFICATION.md` 和 `PHASE_8_REAL_ACQUISITION_BRIDGE_PLAN.md` 补齐书面治理输入；仅在独立任务完成 fake/受控验证后再评估最小试点 |
