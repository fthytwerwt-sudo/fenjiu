...
| R-10 | **PARTIAL / production boundary** | P02-01 至 P02-03 已在 `main` 远端回读；P03-01 仅在远端任务分支完成 synthetic fake staging，尚未合入 `main`；仍缺 RLS、加密、retention、法域、真实 scope、authenticated approval/RBAC、database adapter/driver、真实 parser/OCR/storage 与 production connection | 不得把 local migration、trigger、in-memory policy/read/audit、fake extraction 或 task-branch push 写成生产数据隔离、真实审批、数据合规或真实资料可导入 | 先由控制器审查/集成 P03-01；后续仍只可在新干净 worktree 做 synthetic mapping/quality，真实资料和生产路径须另行获得范围、合规、连接与用户授权 |
