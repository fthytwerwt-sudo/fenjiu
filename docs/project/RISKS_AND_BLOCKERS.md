...
| R-10 | **PARTIAL / production boundary** | P02-01 至 P02-03、P03-01 至 P03-03 与 P04-01 至 P04-02 已在 `main` 远端回读；P04 现仅提供 local synthetic 的 workflow、角色/动作策略和审批版本绑定合同，仍缺 RLS、加密、retention、法域、真实 scope、authenticated identity/authorization、database adapter/driver、真实 parser/OCR/storage、production queue 与 production connection | 不得把 local migration、trigger、in-memory policy/read/audit、fake extraction、mapping fingerprint、isolated approved synthetic version、workflow 或 action-policy contract（动作策略合同）写成生产数据隔离、真实审批、数据合规或真实资料可导入 | 真实资料和生产路径须另行获得范围、合规、连接与用户授权；后续工程任务仍须从新干净 worktree 开始 |
