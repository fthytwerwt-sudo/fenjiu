...
| R-10 | **PARTIAL / production boundary** | P02-01 至 P02-03、P03-01 synthetic staging 与 P03-02 value-free mapping/quality 已在 `main` 远端回读；仍缺 RLS、加密、retention、法域、真实 scope、authenticated approval/RBAC、database adapter/driver、真实 parser/OCR/storage 与 production connection | 不得把 local migration、trigger、in-memory policy/read/audit、fake extraction 或 mapping fingerprint 写成生产数据隔离、真实审批、数据合规或真实资料可导入 | P03-03 仍只可在新干净 worktree 建立 synthetic human approval/publish/refresh contracts；真实资料和生产路径须另行获得范围、合规、连接与用户授权 |
