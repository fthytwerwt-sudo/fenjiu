...
| R-10 | **PARTIAL / production boundary** | P02-01 至 P02-03、P03-01 至 P03-03 与 P04-01 已在 `main` 远端回读；P04-01 仅提供 local simple runner（本地简易运行器）和 synthetic/value-free（合成且不含真实值）恢复合同，仍缺 RLS、加密、retention、法域、真实 scope、authenticated approval/RBAC、database adapter/driver、真实 parser/OCR/storage、production queue 与 production connection | 不得把 local migration、trigger、in-memory policy/read/audit、fake extraction、mapping fingerprint、isolated approved synthetic version 或 workflow contract（工作流合同）写成生产数据隔离、真实审批、数据合规或真实资料可导入 | 真实资料和生产路径须另行获得范围、合规、连接与用户授权；后续工程任务仍须从新干净 worktree 开始 |
