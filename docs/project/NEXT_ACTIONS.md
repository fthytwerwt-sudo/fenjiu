...
1. **P03-02 前置（工程）**：P03-01 已由控制器集成并从 `origin/main` `f92612bf03b5ac740e52d1d56e99f9959369b9fb` 回读；runtime staging atomicity HIGH 已修复且独立专项复审 `APPROVE`。下一张卡须从包含本次状态回填的最新远端 `main` 创建干净 worktree，只执行 synthetic versioned mapping/normalization/quality，保留 source+locator+rule lineage，未知字段/单位/币种/日期保持 manual/blocked，不读真实资料、不写 approved truth。
