...
1. **P03-02 前置（工程）**：P03-01 任务分支代码 `e17196e06380827224a1463f01b53a9975382f22` 已 push/core-file readback，控制器提出的 runtime staging atomicity HIGH 已修复且独立专项复审 `APPROVE`，但尚未合入 `main`。先由控制器审查并集成；随后从新的精确 `main` 创建干净 worktree，只执行 synthetic versioned mapping/normalization/quality，保留 source+locator+rule lineage，未知字段/单位/币种/日期保持 manual/blocked，不读真实资料、不写 approved truth。
