...
- **BLOCKED（Aidge 当前物理 probe）**：`Aidge/2026-04-28:VideoGeneration` 官方 SDK、请求模型、权限点、价格与 5 秒计划已核验；本地 private `.env` 已自动配置北京 region、OSS 公网 Endpoint、Bucket 和 prefix，当前仅缺用户填入两个 AccessKey。`doctor` 将 Aidge/OSS 都标为 credentials absent；Bucket 存在性、地域和 RAM 权限待填 Key 后验证。本轮禁止 Aidge 付费生成。
