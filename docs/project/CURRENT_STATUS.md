...
- **PARTIAL / BLOCKED（Aidge 上游生成完成，本地下载阻断）**：已按用户明确授权只提交 1 次 5 秒、720p、9:16 `VideoGeneration`，最高授权人民币 7 元。Aidge 接受并完成任务、返回输出 URL，证明 credentials、`aidge:VideoGeneration` 权限和上游生成链成立。本地下载因 Provider URL 不满足 credential-free HTTPS policy 而在 MP4 写入前拒绝；task ID 未持久化，ActionTrail 未找到该事件，本地/浏览器暂无可回读终态。当前为 `BLOCKED_AIDGE_OUTPUT_DOWNLOAD_URL_POLICY`，不得写为完整 probe passed。
