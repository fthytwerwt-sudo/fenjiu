...
- **PARTIAL（Aidge 待付费物理 probe）**：`Aidge/2026-04-28:VideoGeneration` 官方 SDK、请求模型、权限点、价格与 5 秒计划已核验；本地 private `.env` 已配置，OSS Bucket 存在且为北京地域，synthetic put/sign/get/delete 通过。Aidge 只读异步查询返回 `InvalidParameter`，表明 credentials/endpoint 被服务端接受；它不证明 `aidge:VideoGeneration` 权限或付费生成成功。本轮未提交收费任务，当前正确状态为 `AIDGE_PROBE_REQUIRED`。
