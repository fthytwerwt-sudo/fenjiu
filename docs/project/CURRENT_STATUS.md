...
- **最近远端验证**：P03-01 工程代码已由 `origin/main` 回读至 `f92612bf03b5ac740e52d1d56e99f9959369b9fb`；默认分支仍不是 `main`，visibility 与同步包最终验证仍待单独完成，不能用本文件编辑或本地 commit 替代。
...
- **PARTIAL（工程 Phase 3 / P03-02 task branch only）**：初始 `codex/p03-02-mapping-quality` 代码提交 `a219463108ca3cf098920d57d17a6b7d8657b01f` 已被控制器 HIGH/not accepted：same ID/version forged profile 可借用 canonical replay proof；独立 reviewer 另发现 quarantined source 或非 `STAGED` lifecycle 仍可 mapping 的 MEDIUM。repair 已将完整 profile fingerprint 绑定进 report、在 registry 写入前验证 prior/current profile-report provenance，并在 mapping 前以 P03-01 enum 阻断 quarantine/non-staged source/job/candidate lineage；任务代码提交 `969a2114c83350a606c917f4c9b8e11c72ca56f0` 已 push/readback，状态为 `task_branch_repair_remote_readback_verified_not_main`，仍未集成 `main`。它不读取或保存真实值，不创建 approved truth，也不改变任何 external flag 或 business gate。
