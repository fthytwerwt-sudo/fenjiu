...
- **BLOCKED（Aidge 当前物理 probe）**：`Aidge/2026-04-28:VideoGeneration` 官方 SDK、请求模型、权限点、价格与 5 秒计划已核验；目标仓库当前缺 `ALIBABA_CLOUD_ACCESS_KEY_ID/SECRET` 和 private OSS 配置，因此真实 Aidge 调用在发送前以 `BLOCKED_AIDGE_CREDENTIALS_ABSENT` 停止，本轮付费调用数为 0。
