...
> **范围边界：** 本报告只记录 local-only runtime、Make 入口、CI 静态验证和扫描兼容；不接数据库连接、不读取环境变量、不加载真实资料、不调用外部 HTTP、模型、采集、发送、发布、报价、付款、订单或退款。API/admin healthcheck 只请求各自固定 loopback health URL，不接受任意 URL 参数。
...
| P00 scanner 兼容 | 已实现 | 扫描器只放行根目录 `.env.example`，`apps/.env.example` 等嵌套 `.env*` 仍按 forbidden path fail-closed。 |
