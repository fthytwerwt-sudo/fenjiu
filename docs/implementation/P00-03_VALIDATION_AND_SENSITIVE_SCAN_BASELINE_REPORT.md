...
| 负例测试 | 已确认 | 新增 stdlib `unittest`，覆盖 forbidden path、AppleDouble、本机绝对路径、高置信 secret、fixture real-data 标记和敏感值不回显。 |
...
3. 对可见路径检测 forbidden path、AppleDouble、`.DS_Store`、大文件和媒体/文档扩展。
4. 对文本文件检测本机绝对路径、高置信 secret-like assignment 和 fixture real-data 标记。
5. 若提供 `--base-sha`，额外检查相对基线变更路径，防止本轮改到 forbidden 区域。
