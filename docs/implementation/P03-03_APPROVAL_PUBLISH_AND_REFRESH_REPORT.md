...
> **状态：task_branch_sixth_repair_remote_readback_verified_not_main**
>
> **执行日期：** 2026-08-07
>
...
>
> **历轮复审结果：** 初始提交 `8f2914844894a0a6bd721d640b99678a89c82e4d`、第二轮 repair `074807b21fc215e4d28d21e524200c5029d70643`、第三轮 repair `918cc8954988f6f723891d2c7989366cee9e692f`、第四轮 repair `c87ec0a817a158900bd8b245f045822139c3175a`、第五轮 repair `b04b524820aa2e58f49365446334ded3a5b2689d` 均被 5.6 只读复审标记 `CHANGES_REQUIRED`，不得作为可接受完成态引用。
>
> **本轮 sixth repair commit / remote readback：** 不在本文件中构造自引用提交号；以最终执行回报中的 pushed commit、remote HEAD 与核心文件 SHA-256 为准。
>
...
- 第六轮 repair 在第五轮基础上继续封闭 reflected write capability：`publish()` / `revoke()` 保持无 closure，旧 closure-bound `commit_transaction` 不再挂到公开方法；raw ledger/outbox/log helpers 即使可从 generated commit function globals 枚举到，也不能通过 direct call、class-attr monkeypatch、FunctionType clone 或 mutable writer-code-set injection 写入 ledger/outbox/log。
- 第六轮移除可 monkeypatch 的 writer registry/guard helper：不存在 `publication_writer_codes` mutable set、code-object closure cell、int writer cell 或 `_require_publisher_commit_writer` module-level no-op switch；写入口用安装时 generated commit code shape 做 local fail-closed 校验。
- 第五轮 repair 在第四轮基础上继续封闭 authority storage：ledger/outbox/transaction-log 的权威状态只存在于闭包内，公开导出的实例没有 `__dict__`、没有 public/private append/commit/get/snapshot/restore 方法、没有可变 state map，类方法默认参数和模块 helper 也不再泄露可直接写入的 capability。
...
- 目标层：修复第五轮遗漏的 reflected raw commit/write capability、writer-code-set/guard-helper 可变授权、class-attr monkeypatch、FunctionType clone 与 helper globals direct-call 绕过；仍只证明 synthetic/internal approval publish contract。
...
- 判断标准层：先写第六轮红灯 regressions 并确认第五轮实现失败路径；修复后 P03 suite、ingestion/contracts/architecture、manual exploit probes、P00 scans、mechanism validation、compile/shell/diff 通过；full regression 的 Docker migration 与 sandbox-blocked localhost bind 项需在 Docker daemon/localhost bind 可用环境补验，业务状态不升级。
...
## 3. 第三、四、五、六轮 findings 修复映射

### 第六轮

| Finding | 修复 |
|---|---|
| HIGH 1 public method closure/globals 泄露 raw commit/write capability，`publish.__closure__` / `revoke.__closure__` 或 `_commit_transaction.__globals__` 可被反射利用 | `publish()` / `revoke()` 恢复为 class body 中的无 closure public entry；旧 closure-bound `commit_transaction`、`publisher_publish`、`publisher_revoke` 从 installer 删除；raw helper direct call、class attr monkeypatch、FunctionType clone 均以 `publication_storage_uninstalled` fail closed，ledger/outbox/log 计数保持 0。 |
| HIGH 1 follow-up writer_codes closure-set bypass | 不使用 `publication_writer_codes` mutable set、code-object closure cell、int writer cell 或 module-level guard helper；新增 regression 会递归查找 writer code set 并尝试注入 `attacker.__code__`，修复后 `writer_code_sets=0`。 |
| HIGH 1 helper monkeypatch bypass | `_require_publisher_commit_writer` 这类可 monkeypatch helper 被移除；写入口直接检查 generated commit caller code shape，不保留可被改成 no-op 的 module-level writer switch。 |
...
- 红灯证据：第六轮 review 在第五轮提交 `b04b524820aa2e58f49365446334ded3a5b2689d` 下复现 `publish/revoke.__closure__` 泄露 `commit_transaction`，closure commit 可写 `ledger=1 outbox=1` 且 `committed_after_request_expiry=True`；后续还复现 mutable `publication_writer_codes` closure-set 可注入 `attacker.__code__` 后绕过 guard。
- 修复后 P03 suite：`python3 -m unittest tests.ingestion.test_approval_publish_and_refresh` 通过 30 项。
- Manual exploit probe：`writer_code_sets=0`，`DIRECT_RAW_BLOCKED ApprovalBoundaryError publication_storage_uninstalled ledger=0`，`MONKEYPATCH_BLOCKED ApprovalBoundaryError publication_storage_uninstalled ledger=0`，`helper_present=False`，`final ledger/outbox=0/0`。
- Manual exploit probe：`H1_public_storage_encapsulation=pass raw_commit_error=publication_storage_uninstalled ledger=0 outbox=0`；公开 ledger/outbox/transaction-log handle 无 `__dict__`、无 append/commit/get/snapshot/restore、无 mutable map，module-level writer helper 不存在，publisher raw commit 也不是可用写入口。
...
- `python3 -m unittest tests.ingestion.test_approval_publish_and_refresh`：通过，30 项。
- `python3 -m unittest discover -s tests/ingestion`：通过，56 项。
- `python3 -m unittest discover -s tests/contracts`：通过，46 项。
- `python3 -m unittest discover -s tests/architecture`：通过，8 项。
- Manual exploit probe（第六轮 reflected writer leak / writer-code-set / direct raw / class-attr monkeypatch / FunctionType clone / no module guard helper，第五轮 HIGH 1-3、第四轮 HIGH 1-3、第三轮 HIGH 1-5、revoke successor、legacy A-F 保留路径）：通过，稳定错误码与计数见第 4 节。
...
- `git diff --check`：通过。
- `python3 -m unittest discover -s tests/regression`：通过，14 项。
- `python3 -m unittest discover -s tests/local_runtime`：通过，8 项。
- `python3 -m unittest discover -s tests/control_plane`：未完整通过；`HealthContractTests.test_http_liveness_is_healthy_and_readiness_is_unavailable` 在当前 sandbox 下创建 `ThreadingHTTPServer(("127.0.0.1", 0), ...)` 被系统拒绝，错误为 `PermissionError: [Errno 1] Operation not permitted`；其余 control-plane 15 项按类/方法拆分运行通过。
- `make regression`：未完整通过；`make compose-config` 已执行到通过，随后 `migration-test` 因当前 Docker daemon 不可用失败，错误为 `migration regression failed: docker daemon unavailable` / Docker API socket permission denied。未申请提权或启动 Docker GUI。
- Docker cleanup readback：未验证；Docker daemon 不可用，未创建可清理的本任务 Compose resources。

## 6. 实际改动文件

- `modules/ingestion/approval.py`：移除 closure-bound public publisher methods 和 mutable writer registry/guard helper；raw helper direct call、class-attr monkeypatch、FunctionType clone 均 fail closed；保留 authority ledger/outbox/log closure-owned state、full committed-result semantic validation、expiry/grant 后 exact replay 顺序、revoke successor independence、defensive snapshot/fingerprint integrity、atomic rollback、full event fingerprint、outbox canonical return validation。
- `tests/ingestion/test_approval_publish_and_refresh.py`：新增第六轮 exploit regression，覆盖 closure/default/global/class/instance reflection、writer-code-set mutation、direct raw helper、class-attr monkeypatch、FunctionType clone 与 no module guard helper；保留第五轮、第四轮、第三轮和旧 A-F regressions。
- `docs/implementation/P03-03_APPROVAL_PUBLISH_AND_REFRESH_REPORT.md`：记录第五轮 `b04b524...` 被拒、第六轮 repair 设计、红灯证据和验证结果。
- `docs/implementation/P03-03_APPROVAL_PUBLISH_AND_REFRESH_HANDOFF.md`：补第六轮 repair note 与 synthetic/internal/P02 边界。

## 7. 事实分级与剩余阻断

- **CONFIRMED（工程）**：P03-03 synthetic/value-free/local-only approval -> decision -> immutable internal publication -> supersede/revoke/successor -> internal invalidation contract 已通过专项、相邻 Python 套件、manual exploit probe、P00 scans、mechanism validation、compile/shell/diff；第六轮 reflected writer leak / writer-code-set / direct raw / class-attr monkeypatch / FunctionType clone regressions 已覆盖，第五轮 storage encapsulation/full replay validation/expiry replay ordering、第四轮 public write/phantom/revoke-reuse/replay regressions 保持不回归。
- **未验证（环境）**：full `make regression` 的 Docker migration replay/negative SQL constraints 与 Docker cleanup readback 因 Docker daemon 不可用未完成；control-plane HTTP bind 单测因 sandbox 禁止 localhost bind 未完成。当前未申请用户审批或外部权限。
...
- Rollback：回退本任务分支 sixth repair commit 即可移除本轮 P03-03 approval module/report/test 改动；不会删除真实业务资料、真实 truth 或外部状态，因为本卡没有创建它们。
