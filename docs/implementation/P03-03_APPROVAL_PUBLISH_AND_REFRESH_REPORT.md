...
> **状态：task_branch_pushed_remote_readback_verified；controller_followup_fix_pending_git_completion**
>
> **执行日期：** 2026-08-08
>
...
- 控制器 follow-up 修复新增 correlation integrity（关联链完整性）与 expiry state（过期状态）：request、decision、version、refresh、audit 必须使用同一个 scope correlation；过期 request 进入 `EXPIRED` 并追加审计，不产生 decision/version/refresh。
...
- `python3 -m unittest tests.ingestion.test_approval_publish_and_refresh`：8 项通过。
- `python3 -m unittest discover -s tests/ingestion`：34 项通过。
- `make regression`：通过；P02 migrations replay 两次、16 类 SQL negative constraints 通过，8 architecture + 14 regression + 8 local-runtime + 16 control-plane + 46 contracts + 34 ingestion tests 通过，隔离 Docker resources 已清理。
...
- **CONFIRMED（工程）**：P03-03 synthetic approval/publish/refresh contract 在任务分支本地验证通过；P03-02 candidate 只有通过人工 decision 后才形成隔离 synthetic approved version；correlation integrity 与 expiry state follow-up 本地专项验证通过。
- **CONFIRMED（工程边界）**：所有 external flags 保持 false；没有新增依赖、网络、数据库、CRM/support/video 实现、真实资料读取或外部执行接口。
- **PARTIAL（Git 完成）**：本文本次 follow-up 更新写入时，追加 commit/push/remote readback 尚未完成；以最终执行回报为准。
