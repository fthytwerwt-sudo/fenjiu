...
这是汾酒在尼泊尔做 `Sales-First` 销售准备的项目。当前阶段是 `SR-1 Sellable Offer Ready`：先收集与核验供应链、合规、账号、收款和履约资料，再决定一个受控渠道触点、统一询盘承接和人工销售闭环；不是已经上线、成交、履约或获得当地许可的证明。
...
3. 当前主线是 Sales-First：`business_gates → sellable offer → controlled channel touchpoint → unified inquiry → human-led sales → order handoff → feedback`。TikTok 是候选内容触点，不是唯一业务中心；多渠道、B2B 联系、自动找客和自动外联不能自动启动。
4. 用户负责渠道/账号运营、内容、商品展示、客户沟通、订单转化、销售数据和市场反馈，以及最终外部执行授权。
...
7A. WhatsApp 不得作为酒类交易通道，Meta commerce channels 不得作为酒类销售通道；技术 capability、platform policy、local legal status 与 project authorization 必须分层核验。
...
若任务混合，先拆分。涉及真实外部执行、酒类合规、账号权限、收款、订单或客户资料时，缺书面授权和证据必须 `BLOCKED`。任何 AI、视频、CRM、crawler、Agent 或自动化都要先写明改善的漏斗步骤、业务 metric、人工 baseline、测量和 Keep/Improve/Stop；不能写明则 `DEFER`。
...
`business_gates（业务闸门）`：SKU、价格、库存、当地主体、品牌授权、账号权限、收款、仓储配送、售后、指定渠道的酒类内容/广告/商品展示/消息边界和用户外部执行授权。
...
在指定渠道的酒类内容/广告/商品展示/消息边界、当地合法主体、品牌/产品资质、SKU、价格、库存、收款、仓储配送、退换货、售后和用户授权没有书面证据前，只允许内部资料整理、清单设计、草稿和核验问题。不得公开发布、投放广告、真实报价、收款、下单、发货或承诺履约。

## 9. 用户说“不对”时

不要急着重写答案。先定位是哪一层不对：

- 项目身份不对：回到 Sales-First 主线，即 `business_gates → sellable offer → controlled channel touchpoint → unified inquiry → human-led sales → order handoff → feedback`。
