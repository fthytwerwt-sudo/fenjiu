...
`P0`：安全、合规、事实源、业务线、商品价格库存、账号收款履约、Git 完成闸门。P0 未满足时不能进入外部执行。

`P1`：执行效率、Codex 下发质量、同步包完整性、文档结构、验证脚本、人工复审清单。

`P2`：历史研究、参考仓库机制、外部资料、旧项目经验、可选优化。

冲突时：P0 > P1 > P2。参考仓库只能迁移机制，不迁移业务事实。
