...
### 6.4 Real Acquisition Source Catalog（真实获客来源目录）

来源等级定义已被落实为两份逐项研究的目录和机器配置：

| 业务线 | 人工阅读版 | Codex 机器配置 | 当前可批准的最小企业发现来源 |
|---|---|---|---|
| `fenjiu_nepal` | [FENJIU_SOURCE_CATALOG.md](FENJIU_SOURCE_CATALOG.md) | [source_catalogs/fenjiu_sources.yaml](source_catalogs/fenjiu_sources.yaml) | `FEN-OSM-POI-NP`，只限 ODbL（开放数据库许可）下的低频企业发现和企业官网交叉验证。 |
| `seafood_nepal` | [SEAFOOD_SOURCE_CATALOG.md](SEAFOOD_SOURCE_CATALOG.md) | [source_catalogs/seafood_sources.yaml](source_catalogs/seafood_sources.yaml) | `SEA-OSM-POI-NP`，同样只限低频企业发现和企业官网交叉验证。 |

目录中其余来源被标为 `CONDITIONAL`、`MANUAL_ONLY` 或 `REJECTED`。尤其是 Google Maps/Places、付费联系人/贸易情报产品、登录绕过和私人资料仍在禁止范围；已批准的企业发现来源也不解除联系人处理、CRM、Gmail、产品/价格/合规或冷链闸门。
...
`gross_score = Opportunity Fit（机会匹配 0-60） + Admission Readiness（准入就绪 0-40）`
`Customer Score = max(0, gross_score - Risk Penalty（风险扣分，0-35 的正数扣减值）)`
...
| 商业可进入性 / 采购流程可进入性 | 0-10 | 有供应商注册、supplier onboarding（供应商准入）、采购部门、招标/采购页、品类采购或 F&B purchasing（餐饮采购）机制的可核验证据。它不同于“有普通企业联系入口”；只有邮箱不得获得本项分数。 |
| **风险扣分** | `0` 至 `35` | 这是从 `gross_score` 中扣除的正数：来源条款不明、数据过期、身份冲突、跨线、冷链缺证据等可触发扣分。DNC、私人数据或禁止来源直接 `BLOCKED`，不只扣分。 |
...
### 8.3 Scoring Contract Validation（评分合同验证）

以下为不含真实客户、联系人或来源值的 synthetic（合成）案例。它验证的是评分数学和硬限制，不是任何真实企业的商业判断。

| 合成案例 | Opportunity Fit | Admission Readiness | 风险扣分 | 数学分数 | 最终状态 / 等级 | 验证点 |
|---|---:|---:|---:|---:|---|---|
| 理想 A 级酒店 | 60 | 40 | 0 | 100 | `A` | 最高分精确等于 100。 |
| 高匹配但来源弱 | 60 | 0 | 0 | 60 | `C` | 来源不够强时不能被产品匹配抬升至 A/B。 |
| 来源很好但产品匹配低 | 15 | 40 | 0 | 55 | `C` | 好来源不能弥补低产品用途匹配。 |
| 有已验证的 20 分风险扣减 | 60 | 40 | 20 | 80 | `B` | 证明正数 `Risk Penalty` 被减去；有风险旗标时不得进入 A。 |
| 低匹配且最大风险扣减 | 15 | 0 | 35 | 0 | `D` | `max(0, …)` 保证最低分不小于 0。 |
| 海鲜客户没有冷链证据 | 60 | 30 | 0 | 90 | `C`（hard cap） | 缺 `freezer_signal`/`cold_storage_signal` 时，现有硬限制优先于高分。 |
| 汾酒客户缺关键合规事实 | 60 | 30 | 0 | 90 | `BLOCKED` | 未批准产品/渠道/合规事实时，不产生可报价、可推广或可联系状态。 |
| DNC 或禁止来源 | 不计算 | 不计算 | 不计算 | 不适用 | `BLOCKED` | `DNC`、删除请求或 X 类来源优先于任何评分。 |

数学合同：

```text
Opportunity Fit maximum = 20 + 15 + 10 + 10 + 5 = 60
Admission Readiness maximum = 10 + 10 + 10 + 10 = 40
gross_score maximum = 100
Customer Score range = 0..100
A = 80..100; B = 65..79; C = 45..64; D = 0..44
```

区间端点没有重叠；`BLOCKED` 是状态机优先级，不是分数等级。未来实现必须保留八个合成案例，并在任何评分逻辑变更后重跑。

### 8.4 建议的 reason code（评分理由代码）
...
SIGNAL_VENDOR_ONBOARDING_PATH
SIGNAL_PROCUREMENT_FUNCTION
RISK_NO_COMMERCIAL_ACCESSIBILITY
SOURCE_A_PRIMARY
...
company_data_storage_allowed: true
...
source_catalog:
  catalog_path: "docs/implementation/source_catalogs/fenjiu_sources.yaml"
  source_catalog_version: "1.0"
  # seafood_nepal 运行时改用 seafood_sources.yaml，不能混用。

source_access_policy:
  enforce_catalog_access_constraints: true
  on_rate_limit_or_policy_uncertainty: "stop"
  on_systematic_enumeration_request: "reject"
...
allowed_source_policy:
  ...
  # 本例是 fenjiu_nepal 的已研究、低频企业发现来源；不是联系人或外联授权。
  approved_source_ids: ["FEN-OSM-POI-NP"]
...
score:
  model_version: "customer-targeting-v2"
...
3. 读取单一业务线的 Source Catalog；只接受其 `use_decision=APPROVED_FOR_DISCOVERY` 且列在运行包 `approved_source_ids` 的来源。不抓取、不绕过限制、不收集个人资料。
4. 在当前最小测试中，只保留瞬时的 `transient_discovery_observation`（临时发现观察）供人工查看；不得写入 candidate store、CRM、日志或 Git。只有来源的 `company_data_storage_status=allowed` 且通过未来独立 P08-RAB 准入合同后，才可建立持久候选记录。
...
| `transient_discovery_ready_for_manual_review` | 当前最小测试发现的公司级观察，仅在内存/人工屏幕中查看，尚未满足持久化条件。 | 人工决定是否补齐来源存储许可；不写 candidate store。 |
...
| 字段 | 本任务的确定内容 |
|---|---|
| `primary_route` | 经批准来源 -> 瞬时公司级发现观察 -> 企业自有官网交叉验证 -> 人工审核；当前不写 candidate store。未来只有 company-data storage 明确 `allowed` 且 P08-RAB 准入合同通过后才进入私有候选/CRM。 |
...
| `required_inputs` | 单业务线运行包、产品范围、城市集群、目标客户类型、`source_catalog_version`、`approved_source_ids`、数据治理/处理依据、业务闸门事实版本。 |
| `required_outputs` | 瞬时发现观察的脱敏人工审核汇总、来源质量、评分理由、风险/阻断计数；不输出/提交 raw 联系人，也不持久化候选。 |
| `execution_entrypoints` | 未来 `P08-RAB-01` 到 `P08-RAB-03` 独立任务卡；当前无需新增代码或 provider。 |
| `validation_commands` | 未来需覆盖 YAML parse、unique source IDs、source/business-line isolation、approved-source access constraints、prohibited-source、score max/min/grade-boundary、DNC/retention/deletion、raw PII/secret/path scan、zero-fetch/zero-send 回归。 |
